...
public class LandingActivity extends BaseActivity implements OnClickListener {
	...
	private class SplashHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case FACEBOOK_RECIEVED_SUCCESS:
				...
				signUp((HashMap<String, String>) msg.getData().getSerializable(
						HASHMAP_BUNDLE));
				break;
			case START_SPINNER:
				startSpinner(true);
	...
	public void signUp(HashMap<String, String> serializableMap) {
		...
		NetworkManager.post(NetworkManager.SIGN_UP_REQUEST_URL,
				new RequestParams(serializableMap), new SignUpListener());
	}
	...
	private void onClickLogin() {
		...
		if (!session.isOpened() && !session.isClosed()) {
			session.openForRead(new Session.OpenRequest(this).setPermissions(
					Arrays.asList("basic_info", "user_birthday")).setCallback(
					statusCallback));
		} else {
			...
			setUserInfoChangedCallback(new UserInfoChangedCallback() {
				...
				public void onUserInfoFetched(GraphUser user) {
					...
					if (user != null) {
						...
						data.put("password", Session.getActiveSession()
								.getAccessToken());
						data.put("isFB", "true");
	...
	class SignUpListener implements RequestListener {
		...
		public void onSuccess(JSONObject json) {
			...
			if (signUpinProg) {
				...
				try {
					...
					signUpinProg = false;
					NetworkManager.post(NetworkManager.ACTIVATE_USER, params,
							this);
				...
				Toast.makeText(LandingActivity.this, "Signed Up",
		...
		public void onFailure(int errorCode) {
			...
			// Here since Facebook login is alaready authenticated by facebook,
			// and hence we have tried to login.
			if (errorCode == NetworkConstants.SignUpRequestConstants.EMAIL_ALREADY_TAKEN) {
				...
				data.put(ParsingConstants.EMAIL,
						PreferenceEngine.getInstance(ctx).getUserEmail());
				data.put(ParsingConstants.PASSWORD, Session.getActiveSession()
						.getAccessToken());
				NetworkManager.post(NetworkManager.SIGN_IN_REQUEST_URL,
						new RequestParams(data), new SignInListener());
			} else {
				removeDialog();
	...
	class SignInListener implements RequestListener {
		...
		public void onSuccess(JSONObject json) {
			...
			removeDialog();
			Toast.makeText(LandingActivity.this, "Signed In",
					Toast.LENGTH_SHORT).show();
			Intent intent = new Intent(LandingActivity.this,
